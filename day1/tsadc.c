...
static int code_to_temp(int code, int *temp)
{
	...
	while (low <= high) {
		mid = (low + high) / 2;
		if (code >= rk3588_code_table[mid].code && code <= rk3588_code_table[mid - 1].code) break;
		else if (code > rk3588_code_table[mid].code) high = mid - 1;
		else low = mid + 1;
	}
	num = rk3588_code_table[mid - 1].temp - rk3588_code_table[mid].temp;
	den = rk3588_code_table[mid - 1].code - rk3588_code_table[mid].code;
	*temp = rk3588_code_table[mid - 1].temp - ((code - rk3588_code_table[mid].code) * num) / den;
	return 0;
}
...
MODULE_DESCRIPTION("Character device driver for RK3588 TSADC");
